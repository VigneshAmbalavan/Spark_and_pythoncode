...
object mostpopularwordintweet {
  def main(args: Array[String]) {

    // Configure Twitter credentials using twitter.txt
    setupTwitter()
    
    // Set up a Spark streaming context named "mostpopularwordintweet" that runs locally using
    // all CPU cores and one-second batches of data
    val ssc = new StreamingContext("local[*]", "mostpopularwordintweet", Seconds(1))
    ...
    val hashtag = statuses.flatMap(status => status.split(' '))
    val hashtag_count=hashtag.map(rdd=>(rdd,1)).reduceByKeyAndWindow(_ + _,_ - _,Seconds(300),Seconds(1))
